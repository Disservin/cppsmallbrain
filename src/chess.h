...
public:
    ...
    std::string printUciMove(Move move);
	...
	// repetition?
	bool isRepetition(int rep = 2);

	// make boardhash
	uint64_t makeBoardHash();
...
void Board::makemove(Move& move){
    ...
    if (piece == makePiece<c>(King)){
        if constexpr (c == White){
            if (source == SQ_E1 && target == SQ_G1 && castlingRights & whiteKingSideCastling){
                ...
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)WhiteRook + 1) + (int)SQ_F1];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)WhiteRook + 1) + (int)SQ_H1];
                ...
                board_hash ^= RANDOM_ARRAY[768+1];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)WhiteRook + 1) + (int)SQ_A1];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)WhiteRook + 1) + (int)SQ_D1];
            ...
            if (source == SQ_E8 && target == SQ_G8 && castlingRights & blackKingSideCastling){
                ...
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)BlackRook + 0) + (int)SQ_F8];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)BlackRook + 0) + (int)SQ_H8];
                ...
                board_hash ^= RANDOM_ARRAY[768+3];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)BlackRook + 0) + (int)SQ_A8];
                board_hash ^= RANDOM_ARRAY[64 * (2 * (int)BlackRook + 0) + (int)SQ_D8];
    ...
    if (move.piece() == Pawn){
        ...
        if (enpassant){
            ...
            board_hash ^= RANDOM_ARRAY[772 + file_of(enpassantSquare)];
        }
    ...
    if (capturedPiece != None) {
        ...
        board_hash ^= RANDOM_ARRAY[64 * (2 * capturedPiece + c) + target];
        halfmoves = 0;
    }
    ...
    if (!move.promoted()){
        ...
        board_hash ^= RANDOM_ARRAY[64 * (2 * piece + c == White) + source];
        board_hash ^= RANDOM_ARRAY[64 * (2 * piece + c == White) + target];
        
    }
    if (move.promoted()){
        removePiece(makePiece<c>(Pawn), source);
        placePiece(piece, target);
        board_hash ^= RANDOM_ARRAY[64 * (2 * makePiece<c>(Pawn) + c == White) + target];
        board_hash ^= RANDOM_ARRAY[64 * (2 * piece + c == White) + target];
        halfmoves = 0;
    }
    // Switch sides
    ...
    gameHistory[fullmoves] = board_hash;
}
...
Moves Board::generateLegalMoves() {
        ...
        while (pawn_mask){
            ...
            while (moves){
                ...
                if (rank_of(target) == 0 || rank_of(target) == 7){
                    ...
                    moveList.Add(Move(source, target, Pawn, 0));
                } 
            }
        }
        while(knight_mask){
            ...
            while(moves){
                ...
                moveList.Add(Move(source, target, Knight, 0));
            }
        }
        while (bishop_mask){
            ...
            while (moves){
                const Square target = poplsb(moves);
                moveList.Add(Move(source, target, Bishop, 0));
            }
        }
        while (rook_mask){
            ...
            while (moves){
                const Square target = poplsb(moves);
                moveList.Add(Move(source, target, Rook, 0));
            }
        }
        while (queen_mask){
            ...
            while (moves){
                const Square target = poplsb(moves);
                moveList.Add(Move(source, target, Queen, 0));
            }
        }
    }
    ...
    while (moves){
        ...
        if (target == SQ_G1 && source == SQ_E1)
            moveList.Add(Move(source, target, King, 0));
        else
            moveList.Add(Move(source, target, King, 0));
        // than this:
