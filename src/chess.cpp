...
std::string Board::printUciMove(Move move){
    std::string uci = squareToString[move.source()] + squareToString[move.target()];
    std::string promoted = "";
    if (move.promoted()) {
         promoted = move.piece() == Queen ? "q" : move.piece() == Rook ? "r" : move.piece() == Bishop ? "b" : "n";
    }
    return uci + promoted;
}
...
bool Board::isRepetition(int rep) {
    int8_t count = 0;
    for (int i = !sideToMove; i < fullmoves && i + 2 < 1024; i += 2) {
        if (gameHistory[i] == board_hash) {
            count++;
        }
        if (count >= rep) return true;
    }
    return false;
}

uint64_t Board::makeBoardHash() {
    Bitboard hash = 0ULL;
    Bitboard white = allPieces<White>();
    Bitboard black = allPieces<Black>();
    bool IsWhite = sideToMove ? 0 : 1;

    while (white) {
        Square sq = bsf(white);
        PieceType piece = PieceType(getPiece(sq)%6);
        if (piece != -1) {
            piece = PieceType(piece * 2 + 1);
    ...
    while (black) {
        ...
        PieceType piece = PieceType(getPiece(sq) % 6);
        if (piece != -1) {
            piece = PieceType(piece * 2);
            hash ^= RANDOM_ARRAY[64 * piece + sq];
        }
        poplsb(black);
    }

    Bitboard ep_hash = 0ULL;
    if (enpassantSquare != NO_SQ) {
        ep_hash = RANDOM_ARRAY[772 + file_of(enpassantSquare)];
    }
    Bitboard turn_hash = IsWhite ? RANDOM_ARRAY[780] : 0;
    Bitboard cast_hash = 0ULL;
...
void Board::makenull() {
    storeInfo[storeCount] = State(enpassantSquare, castlingRights, None);
    storeCount++;
